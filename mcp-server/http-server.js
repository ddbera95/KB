...
// ── HTTP/SSE MCP server — for remote access without SSH ───────────────────────
...
httpServer.listen(PORT, "0.0.0.0", () => {
  console.log(`Mimix MCP HTTP server on port ${PORT}`);
  console.log(`  SSE:    http://0.0.0.0:${PORT}/sse`);
  console.log(`  Health: http://0.0.0.0:${PORT}/health`);
  console.log(`  API:    ${API}  Project: ${PROJECT_ID}`);
});
